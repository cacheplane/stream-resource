...
`provideStreamResource` is the provider factory that registers `@cacheplane/langchain` in Angular's dependency injection system. Call it once inside `bootstrapApplication` (or an `ApplicationConfig`) to configure the transport and any global defaults used by every `streamResource` in your app.
