...
#
# Capture aimock fixtures for the c-a2ui graph by running the standalone
# langgraph dev server against aimock in --record mode. Drives two prompts
# (one per welcome chip) covering the booking-form flows.
#
# Run from repo root:
#   OPENAI_API_KEY=sk-... bash cockpit/chat/a2ui/angular/e2e/scripts/record-c-a2ui.sh
#
# Adapted from record-c-interrupts.sh without the interrupt-resume logic —
# a2ui flows emit inline A2UI JSONL envelopes within a single run.
set -euo pipefail
...
if [[ -z "${OPENAI_API_KEY:-}" ]]; then
  for env_path in examples/chat/python/.env cockpit/chat/a2ui/python/.env; do
    if [[ -f "$env_path" ]]; then
...
AIMOCK_PORT=19997
LANGGRAPH_PORT=5512
FIXTURE_OUT="cockpit/chat/a2ui/angular/e2e/fixtures/c-a2ui.json"
RECORD_DIR="$(pwd)/cockpit/chat/a2ui/angular/e2e/fixtures/.staging"
rm -rf "$RECORD_DIR"
mkdir -p "$RECORD_DIR"
TMP_DIR="$(mktemp -d)"
...
if [[ -f "examples/chat/python/.env" ]]; then
  cp examples/chat/python/.env cockpit/chat/a2ui/python/.env
fi
...
drive_prompt() {
  ...
  run=$(curl -sf -X POST "http://127.0.0.1:$LANGGRAPH_PORT/threads/$thread/runs" \
    ...
    -d "{\"assistant_id\": \"c-a2ui\", \"input\": {\"messages\": [{\"role\": \"user\", \"content\": \"$prompt\"}]}}" \
...
# Welcome-chip prompts from a2ui.component.ts.
drive_prompt "I want to fly LAX to JFK" "lax-jfk"
drive_prompt "I want to fly SFO to SEA" "sfo-sea"
