...
This capability demonstrates how a deep agent decomposes complex tasks into structured plans using the `@ngaf/chat` Angular component library. The `<chat-debug>` component exposes the agent's internal reasoning trace — goal decomposition, sub-task generation, and dependency resolution — so developers can inspect planning decisions in real time.
