...
Key components used: `<chat>` with a thread list sidebar. Each new `streamResource` ref carries the same `userId` namespace so the LangGraph memory store can surface relevant memories regardless of which thread is active.
