...
This capability demonstrates real-time token streaming from a LangGraph agent using the `@cacheplane/chat` Angular component library. The example shows how to wire a `agent` ref into the `<chat>` host component and compose `<chat-messages>`, `<chat-input>`, and `<chat-typing-indicator>` to deliver a responsive, streaming chat experience.
